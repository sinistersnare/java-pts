...
public class FactWriter
{
  ...
  public void writeClassOrInterfaceType(SootClass c)
  {
    ...
    _db.add("HeapAllocationType", constant, _db.addEntity("ClassType", "java.lang.Class"));
  }
  ...
  {
    _db.add("ExceptionHandlerPrevious",
  ...
  public void writeExceptionHandler(SootMethod m, Trap handler, Session session)
  {
    ...
    _db.add("ExceptionHandlerMethod",
    		_db.asEntity(rep),
    		_db.asEntity(_rep.method(m)));

    _db.add("ExceptionHandlerType",
    		_db.asEntity(rep),
    		_db.asEntity(_rep.type(exc)));

    _db.add("ExceptionHandlerFormalParam",
    		_db.asEntity(rep),
    		_db.asEntity(_rep.local(m, caught)));

    _db.add("ExceptionHandlerBegin",
    		_db.asEntity(rep),
    		_db.asIntColumn(String.valueOf(session.getUnitNumber(handler.getBeginUnit()))));

    _db.add("ExceptionHandlerEnd",
    		_db.asEntity(rep),
  ...
  {
	  _db.add("MethodDeclarationException",
			  writeType(exception),
			  _db.addEntity("MethodSignatureRef", _rep.method(m)));
  }
  ...
  private String writeInvokeHelper(SootMethod inMethod, InvokeExpr expr, Session session)
  {
    ...
    if(expr instanceof StaticInvokeExpr)
    {
    	...
    	_db.add("StaticMethodInvocationIn",
    			_db.asEntity(rep),
    			_db.asEntity(_rep.method(inMethod)));
    	_db.add("StaticMethodInvocationSignature",
    			_db.asEntity(rep),
    			_db.asEntity(_rep.signature(expr.getMethod())));
    }
    else if(expr instanceof VirtualInvokeExpr || expr instanceof InterfaceInvokeExpr)
    {
    	_db.add("VirtualMethodInvocationBase",
    			_db.asEntity(rep),
    			_db.addEntity("VarRef", _rep.local(inMethod, (Local) ((InstanceInvokeExpr) expr).getBase())));
    	_db.add("VirtualMethodInvocationSignature",
    			_db.asEntity(rep),
    			_db.asEntity(_rep.signature(expr.getMethod())));
    	_db.add("VirtualMethodInvocationIn",
    			_db.asEntity(rep),
    			_db.asEntity(_rep.method(inMethod)));
    ...
    {
    	_db.add("SpecialMethodInvocationBase",
    			_db.asEntity(rep),
    			_db.addEntity("VarRef", _rep.local(inMethod, (Local) ((InstanceInvokeExpr) expr).getBase())));
    	_db.add("SpecialMethodInvocationSignature",
    			_db.asEntity(rep),
    			_db.asEntity(_rep.signature(expr.getMethod())));
    	_db.add("SpecialMethodInvocationIn",
    			_db.asEntity(rep),
    			_db.asEntity(_rep.method(inMethod)));
    }
    else
